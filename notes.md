...
**`Zustand`** - это небольшая, быстрая и масштабируемая библиотека для управления состоянием. Когда состояние нужно не в одном компоненте, а в несокольких местах приложения, можно вывнести его в глобальное хранилище. Главная ценность что можно передавать без props, на несколько уровней.
*То есть useState - для локального состояния. Общее состояние - Zustand*
* Создается общее хранилище store 
* Внутри store лежат: 
* 1. state - данные 
* 2. actions - функции, которые меняют эти данные.
...
* `count` - это state
* `increment,decrement,reset` - Это actions
* `set(...)` - обновляет состояние store.

### Использвоание store в компоненте
```jsx
function Counter() {
...
// К примеру вот тут |useCounterStore((state) => state.count)| говорится  из всего store мне нужен только count 
```

### persist 

`persist` - используется для автоматического сохранения состояния хранилища (store) в постоянном хранилище браузера (localStorage, sessionStorage или IndexedDB) 
...
`useShalow` - Это хук-обёртка, который возвращает мемоизированный селектор с shallow comparison. В документации Zustand именно так и написано: useShallow returns a memoized version of a selector function using a shallow comparison for memoization.
...
| `git log --oneline`           | Посмотреть короткую историю коммитов                 |
...
| `git checkout -b <имя>`       | Создать новую ветку и сразу перейти в неё            |
